...
class ServicoService {
  ...
  Future<List<Servico>> getServico() async {
    final response = await dio.get('http://192.168.100.112:8080/api/v1/servico');
  ...
  Future<List<Servico>> getServicosAtivos() async {
    final response =
        await dio.get('http://192.168.100.112:8080/api/v1/servico/ativos');
    final data = response.data as List<dynamic>;
    return data.map((json) => Servico.fromMap(json)).toList();
  }
  ...
  Future<List<Servico>> getListarServicosLavacar() async {
    ...
    final response =
        await dio.get('http://192.168.100.112:8080/api/v1/servico/meus-servicos');
    final data = response.data as List<dynamic>;
    return data.map((json) => Servico.fromMap(json)).toList();
  }

  Future<Servico> getServicoById(String id) async {
    final response = await dio.get('http://192.168.100.112:8080/api/v1/servico/$id');
  ...
  Future<void> createServico(String? nome, double? valor, String? tamCarro,
    ...
    await dio.post('http://192.168.100.112:8080/api/v1/servico', data: {
      'nome': nome,
      'valor': valor,
      'tamCarro': tamCarro,
