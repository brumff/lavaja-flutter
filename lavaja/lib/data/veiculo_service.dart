...
class VeiculoService {
  final Dio dio = Dio();
    ...
    final response =
        await dio.get('http://192.168.1.20:8080/api/v1/veiculo');
    final data = response.data as List<dynamic>;
    return data.map((json) => Veiculo.fromMap(json)).toList();
  }
Future<Veiculo> getVeiculoById(String id) async {
    final response = await dio.get('http://192.168.1.20:8080/api/v1/veiculo/$id');
  ...
  Future<void> createVeiculo(String? marca, String? modelo, String? placa, String? cor) async {
    ...
    await dio.post('http://192.168.1.20:8080/api/v1/veiculo', data: {
      'marca': marca,
      'modelo': modelo,
      'placa': placa,
      'cor': cor,
    ...
    await dio.put('http://192.168.1.20:8080/api/v1/veiculo/$id', data: {
      'marca': marca,
      'modelo': modelo,
      'placa': placa,
      'cor': cor,
   ...
   Future<void> deletarVeiculo(int? id) async {
    dio.options.headers = {'authorization': AuthService.token};
    await dio.delete('http://192.168.1.20:8080/api/v1/veiculo/$id');
  }
}